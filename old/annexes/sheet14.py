...
missile = pygame.transform.scale(missile, (MISSILE_WIDTH, MISSILE_HEIGHT))
missiles = []
...
class Asteroid:
    ...
    def rect(self):
        return pygame.Rect(self.x, self.y, asteroid.get_width(), asteroid.get_height())

    def reduce_life(self, damage):
        self.life -= damage
        if self.life <= 0:
            # Just move the asteroid out of the screen, it will not be visible and will be removed on the next frame.
            self.x = -asteroid.get_width()
...
def move_missiles():
    ...
    missiles = [m for m in missiles if m[0] < screen.get_width()]
...
def missile_asteroids_collisions(missiles, asteroids):
    for a in asteroids:
        for i, m in enumerate(missiles):
            if a.rect().collidepoint(m[0] + MISSILE_WIDTH//2, m[1] + MISSILE_HEIGHT//2):
                a.reduce_life(1)
                # Just move the missile out of the screen, it will not be visible and will be removed on the next frame.
                missiles[i] = (screen.get_width(), m[1])
...
while running:
    ...
    missile_asteroids_collisions(missiles, asteroids)

    print("# missiles: ", len(missiles))
